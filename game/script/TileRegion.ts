		...
		getPathHeuristic(type: PathType, weight: number): (fromTile: Tile) => number {
			...
			if (type === PathType.ToArea) {
				...
				return function (tile: Tile) {
					return weight * Pathfinder.diagDistance(tile.x, tile.y, 1, 1, x, y, width, height);
				};
				...
				return function (tile: Tile) {
					return LARGE_NUMBER - weight * Pathfinder.diagDistance(tile.x, tile.y, 1, 1, x, y, width, height);
				};
