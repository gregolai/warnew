...
module Engine.Game {
	...
	export module Pathfinder {
		...
		var _prevPathID = -1;
		var _openList = new BinaryHeap(function (tile: Tile) { return tile.__pathF; });
		
		export function diagDistance(ax: number, ay: number, aWidth: number, aHeight: number, bx: number, by: number, bWidth: number, bHeight: number): number {
